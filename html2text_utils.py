"""
Generic HTML/text/pdf file conversion for batch Streamlit UI.
- Uses DB ConversionFile ORM from store.py.
- Always constructs AustLII-style URL using a configurable CLEAN_BASE (e.g., '/au/cases/cth/HCA' or '/nz/cases/cth/HCA'), not hardcoded, so works for other jurisdictions.
"""
...
def parse_case(filepath, output_rel_path, html_string, clean_base):
    ...
    if not h2_node:
        raise ValueError("Parsing: Missing <h2> title node")
...
def convert_pdf_file(src_path, rel_path, out_base):
    ...
    Strategy:
      1) Try PyPDF2 (fast, bundled in many stacks)
      2) Fallback to pdfminer.six if available (better on some PDFs)
      3) If still empty, raise a clear error (likely scanned/OCR required)
    """
    out_txt_path = os.path.splitext(rel_path)[0] + ".txt"
    dst_fullpath = os.path.join(out_base, out_txt_path)
    ...
    # 2) Fallback: pdfminer.six, if available
    if not text_content.strip():
        try:
            from pdfminer.high_level import extract_text as pdfminer_extract_text
            text_content = pdfminer_extract_text(src_path) or ""
        except Exception:
            # ignore if not installed or fails; will handle below
            pass

    # 3) Optional OCR fallback (Tesseract) if enabled, else raise
    if not text_content.strip():
        ocr_enabled = os.environ.get("COGNEO_PDF_OCR", "").lower() in ("1", "true", "yes", "on")
        if ocr_enabled:
            dpi = int(os.environ.get("COGNEO_PDF_OCR_DPI", "300"))
            try:
                import pytesseract
                ocr_text_parts = []

                # Try pdf2image first (requires poppler installed)
                try:
                    ...
                    try:
                        ...
                        # leave text_content empty if both OCR paths fail
                        pass
            except Exception:
                # pytesseract not installed or failed
                pass

        if not text_content.strip():
            raise Exception(
                "No text extracted from PDF. The file may be scanned or image-only. "
                "Enable OCR fallback by setting COGNEO_PDF_OCR=1 and install dependencies "
                "(pdfminer.six, pytesseract, and either pdf2image+poppler or PyMuPDF)."
            )
